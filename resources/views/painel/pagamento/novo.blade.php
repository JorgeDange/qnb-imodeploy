...
@section('title', 'Novo Pagamento — Painel | QNB-Imobiliária')
...
<form action="{{ route('painel.pagamento.salvar') }}" method="POST" enctype="multipart/form-data" class="ul-painel-form">
    @csrf
            ...
            <div class="row">
                ...
                <div class="col-lg-4 col-md-6 mb-3">
                    <label style="display:block;cursor:pointer;">
                        <input type="radio" name="plano_id" value="{{ $plano->id }}" {{ $loop->first ? 'checked' : '' }} style="display:none;" required>
                        <div class="ul-painel-card" style="text-align:center;transition:all 0.2s;{{ $planoAtual && $planoAtual->plano_id == $plano->id ? 'border:2px solid var(--ul-primary);opacity:0.6;' : '' }}" onclick="this.querySelector('input').checked=true;document.querySelectorAll('.ul-painel-card[style*=border]').forEach(c=>{if(c!==this)c.style.border='';});this.style.border='2px solid var(--ul-primary)';">
                            <h4 style="margin:0 0 8px;">{{ $plano->nome }}</h4>
                            <div style="font-size:28px;font-weight:700;color:var(--ul-primary);">{{ number_format($plano->preco, 0, ',', '.') }} <small style="font-size:14px;color:#999;">{{ $plano->moeda }}</small></div>
                            <div style="font-size:13px;color:#666;margin:8px 0;">{{ $plano->dias_validade }} dias · {{ $plano->posts_limite }} posts</div>
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select name="metodo" required>
                            ...
                            <option value="transferencia" {{ old('metodo') == 'transferencia' ? 'selected' : '' }}>Transferência Bancária</option>
                            <option value="multicaixa" {{ old('metodo') == 'multicaixa' ? 'selected' : '' }}>Multicaixa Express</option>
                            <option value="dinheiro" {{ old('metodo') == 'dinheiro' ? 'selected' : '' }}>Dinheiro</option>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="text" name="referencia" value="{{ old('referencia') }}" placeholder="Nº da transferência, recibo, etc.">
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group">
    ...
    <div class="ul-painel-card">
        <div class="ul-painel-form-seccao">
            ...
            <a href="{{ route('painel.ativar-plano') }}" class="ul-btn" style="margin-left:10px;">Cancelar</a>
        </div>
    </div>
</form>
@endsection
